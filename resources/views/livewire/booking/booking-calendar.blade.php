            ...
            <div class="mb-4 bg-white rounded-lg border border-slate-200 shadow-sm px-4 py-3">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <div class="flex items-center gap-2">
                        <button type="button" wire:click="previousDay" aria-label="Hari sebelumnya" class="inline-flex items-center justify-center w-9 h-9 rounded-md border border-slate-300 text-slate-600 hover:bg-slate-50 transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                            </svg>
                        </button>

                        <input type="date" wire:model.live="selectedDate" aria-label="Pilih tanggal" class="rounded-md border-slate-300 text-slate-900 shadow-sm focus:border-bpjs-blue-500 focus:ring-bpjs-blue-500 sm:text-sm" />

                        <button type="button" wire:click="nextDay" aria-label="Hari berikutnya" class="inline-flex items-center justify-center w-9 h-9 rounded-md border border-slate-300 text-slate-600 hover:bg-slate-50 transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                            </svg>
                        </button>

                        <button type="button" wire:click="setToday" class="inline-flex items-center px-3 py-2 rounded-md border border-slate-300 text-sm font-medium text-slate-700 hover:bg-slate-50 transition-colors">
                            Hari Ini
                        </button>
                    </div>
                ...
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    @foreach ($allRooms as $room)
                        ...
                        <button type="button" wire:click="toggleRoom({{ $room->id }})" aria-pressed="{{ $isActive ? 'true' : 'false' }}" @class([
                            ...
                            'bg-bpjs-blue-50 text-bpjs-blue-700 border border-bpjs-blue-200 hover:bg-bpjs-blue-100' => $isActive,
                            'bg-slate-50 text-slate-500 border border-slate-200 hover:bg-slate-100' => ! $isActive,
                        ])>
                            {{ $room->name }}
                        </button>
                    @endforeach
                    @if (! empty($roomFilterIds))
                        <button type="button" wire:click="$set('roomFilterIds', [])" class="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium text-slate-500 hover:text-slate-700 transition-colors ml-2">
                            Reset filter
                        </button>
                    @endif
                </div>
            ...
            @else
                ...
                <div class="hidden md:block bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
                    <div class="overflow-x-auto">
                        <div class="grid min-w-fit" style="grid-template-columns: 80px repeat({{ count($rooms) }}, minmax(160px, 1fr)); grid-template-rows: 40px repeat({{ count($timeWindow['slots']) }}, 40px);">
                            <div class="bg-slate-100 border-b border-r border-slate-200 sticky left-0 z-20" style="grid-column: 1; grid-row: 1;"></div>

                            @foreach ($rooms as $i => $room)
                                <div class="bg-slate-100 border-b border-r border-slate-200 px-3 py-2 flex flex-col justify-center" style="grid-column: {{ $i + 2 }}; grid-row: 1;">
                            ...
                            @foreach ($timeWindow['slots'] as $rowIdx => $slot)
                                <div @class([
                                    'border-r border-b border-slate-100 px-2 py-1 text-[11px] font-mono text-slate-500 sticky left-0 z-10',
                                ...
                                @foreach ($rooms as $i => $room)
                                    <a href="{{ $this->emptyCellHref($room->id, $slot) }}" wire:navigate aria-label="Buat reservasi {{ $room->name }} pukul {{ $slot }}" @class([
                                        'border-r border-b border-slate-100 transition-colors group flex items-center justify-center hover:bg-bpjs-blue-50',
                                        'bg-slate-50' => $rowIdx % 2 === 0,
                                        'bg-white' => $rowIdx % 2 !== 0,
                            ...
                            @foreach ($bookings as $booking)
                                ...
                                @if ($position !== null && $roomIndex !== false)
                                    <div wire:key="booking-block-{{ $booking->id }}" @class([
                                        'relative z-10 m-0.5 rounded-md border px-2 py-1 overflow-hidden flex flex-col justify-start',
                                        'bg-bpjs-green-50 border-bpjs-green-300 text-bpjs-green-900' => $isApproved,
                                        'bg-amber-50 border-amber-300 text-amber-900' => ! $isApproved,
                ...
                <div class="md:hidden space-y-3">
                    <a href="{{ route('bookings.new') }}" wire:navigate class="flex items-center justify-center gap-2 w-full px-4 py-3 bg-bpjs-blue-500 hover:bg-bpjs-blue-600 text-white text-sm font-medium rounded-lg shadow-sm transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                        </svg>
                        Buat Reservasi Baru
                    ...
                    @foreach ($rooms as $room)
                        ...
                        <section wire:key="room-mobile-{{ $room->id }}" class="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
                            ...
                            @if ($roomBookings->isEmpty())
                                <div class="px-4 py-4">
                                    ...
                                    <a href="{{ route('bookings.new', ['room_id' => $room->id]) }}" wire:navigate class="mt-2 inline-flex items-center gap-1 text-xs font-medium text-bpjs-blue-600 hover:text-bpjs-blue-700">
                                        ...
                                        <li wire:key="mobile-booking-{{ $booking->id }}" class="px-4 py-3">
                                            <div class="flex items-start justify-between gap-3">
                                                ...
                                                <span @class([
                                                    ...
                                                    'bg-bpjs-green-50 text-bpjs-green-800 border border-bpjs-green-200' => $isApproved,
                                                    'bg-amber-50 text-amber-800 border border-amber-200' => ! $isApproved,
                                                ])>
