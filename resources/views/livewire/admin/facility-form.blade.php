        ...
        <div class="card bpjs-rise">
            <div class="card--pad space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <x-bpjs.field :label="__('Kode Fasilitas')" req for="code" :error="$errors->first('code')">
                        <input wire:model="code" type="text" id="code" placeholder="projector"
                               class="input @error('code') input--err @enderror" />
                    </x-bpjs.field>

                    <x-bpjs.field :label="__('Nama Fasilitas')" req for="name" :error="$errors->first('name')">
                        <input wire:model="name" type="text" id="name" placeholder="Proyektor"
                               class="input @error('name') input--err @enderror" />
                    </x-bpjs.field>

                    <x-bpjs.field :label="__('Kategori')" for="category" :error="$errors->first('category')">
                    ...
                    <x-bpjs.field :label="__('Ikon (opsional)')" for="icon" :error="$errors->first('icon')">
                ...
                <label class="flex items-center cursor-pointer gap-2">
                    ...
                    <span class="text-sm text-slate-700">{{ __('Fasilitas aktif') }}</span>
                </label>
            </div>

            <div class="modal__foot" style="border-radius: 0 0 16px 16px;">
                <x-bpjs.button variant="ghost" :href="route('admin.facilities.index')" wire:navigate>{{ __('Batal') }}</x-bpjs.button>
                <x-bpjs.button type="submit" wire:loading.attr="disabled" wire:target="save">
                    <span wire:loading.remove wire:target="save">{{ $isEditMode ? __('Simpan Perubahan') : __('Buat Fasilitas') }}</span>
