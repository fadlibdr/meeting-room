                ...
                <x-bpjs.field :label="__('Cari kode atau nama')" for="search">
                    <div class="relative">
                        ...
                        <input wire:model.live.debounce.300ms="search" type="text" id="search"
                               placeholder="{{ __('contoh: BIRO-UMUM') }}" class="input pl-10" />
                    </div>
                </x-bpjs.field>
            </div>
            <div>
                            ...
                            <div class="inline-flex items-center justify-end gap-3">
                                @hasPermission('users.update')
                                    ...
                                    <button wire:click="toggleActive({{ $unit->id }})"
                                        ...
                                        {{ $unit->is_active ? __('Nonaktifkan') : __('Aktifkan') }}
                                    </button>
                                @endhasPermission
                            </div>
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="text-center text-slate-500" style="padding: 48px 16px;">
                            {{ __('Belum ada unit. Klik "Tambah Unit" untuk membuat yang pertama.') }}
                        </td>
                    </tr>
                @endforelse
            </tbody>
